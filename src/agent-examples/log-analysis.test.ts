...
`);
  ...
  it('should find all WARN entries in app log', async () => {
    ...
    expect(result.stdout).toBe('2024-01-15 10:10:01 WARN  Retrying connection\n');
    expect(result.stderr).toBe('');
    expect(result.exitCode).toBe(0);
  });
...
`);
  ...
  it('should get recent log entries', async () => {
    ...
    expect(result.stdout).toBe(`2024-01-15 10:20:45 ERROR Auth failed user@example.com
2024-01-15 10:30:15 ERROR NullPointerException
`);
    expect(result.stderr).toBe('');
    expect(result.exitCode).toBe(0);
